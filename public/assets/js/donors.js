document.addEventListener('DOMContentLoaded', function() {
  ...
  filterButtons.forEach(button => {
    button.addEventListener('click', function() {
      ...
      // Filtrar las tarjetas de donadores
      filterDonors(filter);
    });
  });
  ...
  // Insertar el botón de ordenamiento junto a los filtros
  const filtersContainer = document.querySelector('.donors-filters');
  if (filtersContainer) {
    filtersContainer.appendChild(sortButton);
  }
  
  let sortOrder = 'desc'; // 'desc' para mayor a menor, 'asc' para menor a mayor
  
  sortButton.addEventListener('click', function() {
    const donorsList = document.querySelector('.donors-list');
    const cards = Array.from(donorCards).filter(card => !card.classList.contains('hidden'));
    
    // Ordenar las tarjetas según la cantidad
    cards.sort((a, b) => {
      const amountA = parseInt(a.querySelector('.donor-amount').textContent.replace(/[^\d]/g, ''));
      const amountB = parseInt(b.querySelector('.donor-amount').textContent.replace(/[^\d]/g, ''));
    ...
    sortButton.textContent = sortOrder === 'desc' ? 'Ordenar por cantidad (↓)' : 'Ordenar por cantidad (↑)';
  });
  ...
  // Insertar el campo de búsqueda antes de los filtros
  const filtersSection = document.querySelector('.donors-filters').closest('.card');
  if (filtersSection) {
  ...
  searchInput.addEventListener('input', function() {
    ...
    donorCards.forEach(card => {
      const donorName = card.querySelector('.donor-name').textContent.toLowerCase();
      const donorMessage = card.querySelector('.donor-message');
      const messageText = donorMessage ? donorMessage.textContent.toLowerCase() : '';
    ...
    // Si hay un filtro activo, aplicarlo también después de la búsqueda
  ...
  // Funcionalidad de estadísticas en tiempo real
  function updateStats() {
    const visibleCards = Array.from(donorCards).filter(card => !card.classList.contains('hidden'));
    const totalDonors = visibleCards.length;
    const totalAmount = visibleCards.reduce((sum, card) => {
      const amount = parseInt(card.querySelector('.donor-amount').textContent.replace(/[^\d]/g, ''));
      return sum + amount;
    }, 0);
    const averageAmount = totalDonors > 0 ? Math.round(totalAmount / totalDonors) : 0;
    ...
    if (statValues.length >= 3) {
      ...
      statValues[1].textContent = `${totalAmount.toLocaleString('es-ES')}&nbsp;€`;
      statValues[2].textContent = `${averageAmount.toLocaleString('es-ES')}&nbsp;€`;
    }
  }
  
  // Actualizar estadísticas cuando se aplican filtros o búsqueda
  filterButtons.forEach(button => {
  ...
  // Funcionalidad de compartir donadores destacados
